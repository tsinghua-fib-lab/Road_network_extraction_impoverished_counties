...
for district in district_list:
    ...
    if os.path.exists('haze_csv/'+ district+'_haze_status_2017.csv'):
        continue
    ...
    for i in img_list:
        ...
        if np.mean(img[:,:,0]) <=0.3 or  np.mean(img[:,:,1]) <=0.3 or np.mean(img[:,:,2]) <=0.3:
            haze_list.append(1)
        else:
            haze_list.append(0)

        img_name_list.append(i.split('/')[-1].split('.')[0])
        np1.append(np.mean(img[:,:,0]))
        np2.append(np.mean(img[:,:,1]))
        np3.append(np.mean(img[:,:,2]))
    
    pd_dict = pd.DataFrame({'img_name':img_name_list,'np1':np1,'np2':np2,'np3':np3,'haze':haze_list})
    pd_dict.to_csv('haze_csv/'+ district+'_haze_status_2017.csv')
