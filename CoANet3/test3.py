...
def main():
    ...
    parser.add_argument('--district', type=str, default=False,
                        help='name of the district')
    
    args = parser.parse_args()
    print(args)

    file_list_tmp = glob.glob('../zl16_images_20_plus_20/2021/'+args.district+'/*.png')
    file_list = file_list_tmp

    print(args.district, len(file_list))

    short_f_list = [x.split('/')[-1].split('.')[0] for x in file_list]
    print(short_f_list[:5])

    with open('../zl16_images_20_plus_20/2021/'+args.district+'_val.txt',"w") as f:
        for k in short_f_list:
            f.write(k)
            f.write('\n')
    ...
    test_loader, nclass = make_data_loader(args, **kwargs) #
    ...
    out_path = os.path.join(args.out_path, args.district+'_out_imgs_1300/')
    ...
    for i, sample in enumerate(tbar):
        ...
        if os.path.exists(out_path + img_name + '_pred' + '.png'):
            continue
